...
// SAFE ELEMENT LOADER: Prevents crash if HTML IDs are missing
const getEl = (id) => document.getElementById(id) || { textContent: "", innerHTML: "", style: {}, classList: { add:()=>{}, remove:()=>{}, toggle:()=>{} }, setAttribute:()=>{}, removeAttribute:()=>{} };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    main().catch(err => {
        ...
        document.body.innerHTML = `<div style="color:maroon; padding:50px; text-align:center; font-family:sans-serif;">
            <h2>System Error</h2>
            <p>${err.message}</p>
            <button onclick="location.reload()" style="background:maroon; color:white; padding:10px 20px; border-radius:10px; border:none; cursor:pointer;">Retry Load</button>
        </div>`;
    });
});
...
function initTheme() {
    ...
    if (els.themeToggle && els.themeToggle.onclick !== undefined) {
...
// SMART DATA FETCHING: Tries multiple paths to prevent white screen
async function smartFetch(fileName) {
    const paths = [fileName, `assets/data/${fileName}`, `../assets/data/${fileName}`, `./assets/data/${fileName}`];
    ...
    throw new Error(`File Not Found: ${fileName}. Check your folder structure.`);
}
