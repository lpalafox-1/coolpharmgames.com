...
- The active roadmap is the ordered Phase 2 Facelift lane in `docs/phase-roadmap.md`, which is the single source of truth for task status — read it rather than trusting any task name restated here, since a restated name goes stale on every merge. Exactly one task is active at a time, either `READY` or `IN PROGRESS`; do not select stale Fall work or infer readiness from satisfied dependencies. A task becomes `DONE` only when the owner merges its branch, and that merge is also what promotes the next task to `READY`. As of PR #67 (2026-09-03) P2F-08 is `DONE` and P2F-09 is `READY`; verify against the roadmap before acting on that.
